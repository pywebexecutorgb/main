<template>
  <div class="block">
      ...
      <div v-if="isHistoryCount">
        ...
        <div class="buttons btn-group" role="group">
          <button
            ...
            @click="getUserHistory(history.previous)"
          >Prev</button>
          <button
            type="button"
            class="btn btn-outline-secondary"
            :disabled="!history.next"
            @click="getUserHistory(history.next)"
          >Next</button>
        </div>
      </div>
    ...
    <div v-else class="message error">You must be logged in.</div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    isHistoryCount() {
      return (
        this.history &&
        this.history.hasOwnProperty("count") &&
