    ...
    <div v-show="showLogin" class="user-profile">
      ...
      <div v-else>
        <ul>
          ...
          <li>
            <button @click="loginUser()" class="user-buttons btn btn-outline-success">Login</button>
          </li>
          <li>
            <button
              @click="resetPasswordUser()"
              class="user-buttons btn btn-outline-secondary"
...
export default {
  ...
  methods: {
    ...
    getUserInfo() {
      ...
      this.getJSON(url, { credentials: "include" }).then(response => {
    ...
    async loginUser() {
      if (
        !this.isValidInput("authEmail") ||
        !this.isValidInput("authPassword")
      ) {
        return false;
      }
